...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!attendance || !attendance.isPresent) {
      return NextResponse.json({ error: 'You must have attended the training to provide feedback' }, { status: 400 })
    }
    ...
    await sendCertificateNotification(user.email, training.title, certificateUrl)
    ...
    return NextResponse.json({ 
      feedback,
      certificate,
      message: 'Feedback submitted successfully. Your certificate is now available!' 
    }, { status: 201 })
  } catch (error) {
