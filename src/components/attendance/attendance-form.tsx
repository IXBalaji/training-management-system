...
export default function AttendanceForm({ trainingId, enrollments, isLocked }: AttendanceFormProps) {
  ...
  useEffect(() => {
    ...
    enrollments.forEach(enrollment => {
      if (enrollment.attendance.length > 0) {
        initialAttendance[enrollment.user.id] = enrollment.attendance[0].isPresent
      } else {
    ...
    <div className="space-y-4">
        ...
        {enrollments.map((enrollment) => (
          ...
          >
            ...
            <div className="flex space-x-2">
              ...
              <button
                onClick={() => handleAttendanceChange(enrollment.user.id, false)}
                disabled={isSaved}
                className={`p-2 rounded-full transition-colors ${
                  !attendance[enrollment.user.id]
      ...
      {isSaved && (
        <div className="bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-md text-sm">
          Attendance has been saved and locked. Feedback requests will be sent to attendees.
        </div>
      )}
    </div>
  )
